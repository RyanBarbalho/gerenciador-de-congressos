...
class Materia:
    ...
    precisa_lab: bool
    materiais_necessarios: List[str]  # Ex: ["computadores", "projetor"]
...
class AlocadorSalas:
    ...
    def _eh_compativel(self, materia: Materia, sala: Sala) -> bool:
        ...
        if materia.precisa_lab and sala.tipo != TipoSala.LABORATORIO:
            return False
            
        # Se a matéria não precisa de laboratório, pode ir para qualquer tipo
        # Verifica se a sala tem os materiais necessários
        for material in materia.materiais_necessarios:
            if material not in sala.materiais_disponiveis:
                return False
